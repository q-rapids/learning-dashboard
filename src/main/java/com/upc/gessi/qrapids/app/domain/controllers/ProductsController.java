	...
	public List<DTOProduct> getProducts() throws Exception {
		...
		for (Product p : productsBD) {
			...
			for (Project proj : p.getProjects()) {
				DTOProject project = new DTOProject(proj.getId(), proj.getExternalId(), proj.getName(), proj.getDescription(), proj.getLogo(), proj.getActive(), proj.getBacklogId(), proj.getTaigaURL(), proj.getGithubURL());
